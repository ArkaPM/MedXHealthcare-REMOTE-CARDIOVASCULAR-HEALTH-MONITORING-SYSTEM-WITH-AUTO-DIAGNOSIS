...
def run_kfold(clf):
    ...
    print ("By RFNN and KFold       	::",predictions3)

    #new_predict
    for val in predictions3:
        if val == 0:
            new_predict.append("You are fine! But you can make a visit within a month.")
        else:
            new_predict.append("Please visit doctor ASAP!! Call 108 for Emergency Ambulance service.")

    print ("Strings       ::", new_predict)


    employ_data = open('./HeartCondition_Data.csv', 'w')

    csvwriter = csv.writer(employ_data)
    K=['Auto-diagnosed Heart Condition']
    csvwriter.writerow(K)

    csvwriter.writerow(new_predict)

    employ_data.close()

    print ("Heart Condition Recorded successfully")
